...
## [Unreleased]
### Added
- Added `apache-spark-pyspark/diagrams/spark-job-execution.excalidraw` — end-to-end Spark job execution diagram (8 panels: physical architecture, query planning, execution hierarchy, staged DataFrame example with shuffle boundaries, shuffle anatomy, scheduling/retries, executor JVM memory model, AQE + recap), referenced from `spark-concepts-execution-architecture.md`
- Added opencode project config (`opencode.json`) and `.opencode/agent/repo-improver.md` subagent for continuous repo improvement and maintenance
- Expanded `emr/notes.md` to 1288 lines (+1157) — 15 sections covering architecture (nodes, YARN deep dive, EMRFS internals, HDFS strategy), cluster lifecycle (transient/long-running/managed scaling deep dive with step orchestration), instance selection methodology and Graviton migration guide, security & networking (IAM least-privilege, VPC endpoints, encryption, Kerberos/Ranger), monitoring & debugging (CloudWatch metrics, Spark History Server, debug playbook), orchestration (Steps API, Step Functions, bootstrap actions, custom AMIs), performance tuning (Spark config profile, shuffle tuning, EBS tuning), production patterns & anti-patterns with failure mode catalog, lakehouse integration (Iceberg, Delta, Glue Catalog), EMR on EKS and Serverless depth, disaster recovery, migration patterns (on-prem Hadoop, version upgrades), and 6 interview answers
